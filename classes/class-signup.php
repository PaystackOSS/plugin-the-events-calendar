...
class Signup extends Abstract_Signup {
	 ...
	 * @inheritDoc
	 */
	public $template_folder = 'src/admin-views/settings/tickets-commerce/paystack';
	...
	public function ajax_refresh_connect_url() {
		...
		if ( ! wp_verify_nonce( $nonce, 'tec-tickets-commerce-gateway-paystack-refresh-connect-url' ) ) {
			wp_send_json_error( new \WP_Error( 'tec-tickets-commerce-paystack-nonce-problem' ) );
			return;
		}
		...
		if ( empty( $new_url ) ) {
			wp_send_json_error( new \WP_Error( 'tec-tickets-commerce-paystack-refresh-connect-url-error' ) );
			return;
		}
	...
	public function get_errors_from_on_boarded_data() {
		...
		$saved_merchant_id = $merchant->get_merchant_id_in_paystack();
		...
		if ( ! $payments_receivable ) {
			$error_messages[] = esc_html__( 'Set up an account to receive payment from paystack', 'event-tickets' );
		}
		...
		if ( array_diff( [ 'products', 'capabilities' ], array_keys( $seller_status ) ) ) {
			$error_messages[] = esc_html__( 'Your account was expected to be able to accept custom payments, but is not. Please make sure your
				account country matches the country setting. If the problem persists, please contact paystack.', 'event-tickets' );
		...
		if ( ! $has_custom_payments ) {
			$error_messages[] = esc_html__( 'Reach out to paystack to enable PPCP_CUSTOM for your account', 'event-tickets' );
		}
		...
		if ( ! empty( $invalid_capabilities ) ) {
			$error_messages[] = esc_html__( 'Reach out to paystack to resolve the following capabilities:', 'event-tickets' ) . ' ' . implode( ', ', $invalid_capabilities );
		}
	...
	/**
	 * When toggling between paystack test mode and live mode, we need to re-generate
